...
public class LockManager {
    ...
    public boolean Lock(int xid, String data, TransactionLockObject.LockType lockType) throws DeadlockException {
        ...
        System.err.println("LM::lock(" + xid + ", " + data + ", " + lockType + ") called");
        ...
        try {
                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                            TransactionLockObject oldTrx = new TransactionLockObject(xid, data, TransactionLockObject.LockType.LOCK_READ);
                            DataLockObject oldDataRead = new DataLockObject(xid, data, TransactionLockObject.LockType.LOCK_READ);

                            lockTable.remove(oldTrx);
                            lockTable.remove(oldDataRead);

                            lockTable.add(xLockObject);
                            lockTable.add(dataLockObject);

                            System.err.println("LM::lock(" + xid + ", " + data + ", " + lockType + ") converted");
                        } else {
                            // Lock request that is not lock conversion
                            this.lockTable.add(xLockObject);
                            this.lockTable.add(dataLockObject);

                            System.err.println("LM::lock(" + xid + ", " + data + ", " + lockType + ") granted");
                        }
                    }
                }
                if (bConflict) {
            ...
            System.err.println("LM::lock(" + xid + ", " + data + ", " + lockType + ") " + redundantlockrequest.getLocalizedMessage());
            return true;
        }
    ...
    public boolean UnlockAll(int xid) {
        ...
        synchronized (this.lockTable) {
            ...
            for (int i = (size - 1); i >= 0; i--) {
                ...
                System.err.println("LM::unlock(" + xid + ", " + xLockObject.getDataName() + ", " + xLockObject.getLockType() + ") unlocked");
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (dataLockObject.getXId() == l_dataLockObject.getXId()) {
                ...
                if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_READ) {
                    ...
                    // if already has write throw exception
                    ...
                    if (l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_READ) {
                        ...
                        for (Object o : vect) {
                            if (((DataLockObject) o).getXId() != dataLockObject.getXId()) {
                                System.err.println("LM::lockConflict(" + dataLockObject.getXId() + ", " + dataLockObject.getDataName() + ") Want WRITE, someone has READ");
                                // conflict exists
                                return true;
                            }
                        }
                ...
                if (l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE) {
                    ...
                    System.err.println("LM::lockConflict(" + dataLockObject.getXId() + ", " + dataLockObject.getDataName() + ") Want READ, someone has WRITE");
                    return true;
                }
            } else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE) {
                // Transaction is requesting a WRITE lock and some other transaction has either
                // a READ or a WRITE lock on it ==> conflict
                System.err.println("LM::lockConflict(" + dataLockObject.getXId() + ", " + dataLockObject.getDataName() + ") Want WRITE, someone has READ or WRITE");
                return true;
            }
        }
    ...
    private void WaitLock(DataLockObject dataLockObject) throws DeadlockException {
        System.err.println("LM::waitLock(" + dataLockObject.getXId() + ", " + dataLockObject.getDataName() + ", " + dataLockObject.getLockType() + ") called");
    ...
    // CleanupDeadlock cleans up stampTable and waitTable, and throws LockManager.DeadlockException
    private void cleanupDeadlock(TimeObject timeObject, WaitLockObject waitLockObject) throws DeadlockException {
        System.err.println("LM::deadlock(" + waitLockObject.getXId() + ", " + waitLockObject.getDataName() + ", " + waitLockObject.getLockType() + ") called");
        synchronized (this.stampTable) {
            synchronized (this.waitTable) {
                this.stampTable.remove(timeObject);
                this.waitTable.remove(waitLockObject);
